...
Страницы листаются **автоматически**. Механизм см. 7.5.
...
### 7.6. Прогресс-бар (нижняя панель)
...
- Клавиатура всегда открыта (кроме режима настроек с жанрами).
- Плейсхолдер поиска: `"Поиск авторов…"` / `"Поиск по названию…"`.
- Фильтрация: первые 3 символа — `LIKE 'запрос%'`, далее — `LIKE '%запрос%'` (FTS5 опционально).
